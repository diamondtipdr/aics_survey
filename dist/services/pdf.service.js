    ...
    for (const line of lines) {
        ...
        if (headingMatch) {
            ...
            if (heading.startsWith('Quick Win')) {
                ...
                htmlParts.push(`<h3 class="quick-win-heading">⚡ Quick Win — ${escapeHtml(pillarName)}</h3>`);
            }
            else {
                inQuickWin = false;
                ...
                if (scoreMatch) {
                    htmlParts.push(`<h3 class="pillar-heading">${escapeHtml(pillarName)} <span class="pillar-heading-score">${escapeHtml(scoreMatch[0])}</span></h3>`);
                }
                else {
