 ...
 */
async function sendEmail(to, subject, htmlBody, pdfBuffer, pdfFilename, ctx) {
    const logger = (0, logger_1.withContext)(ctx);
    if (!config_1.config.mailgunApiKey || !config_1.config.mailgunDomain) {
        logger.warn('Mailgun not configured — skipping email dispatch');
        return;
    }
    const form = new form_data_1.default();
    form.append('from', `AICS Auditoría <noreply@${config_1.config.mailgunDomain}>`);
    form.append('to', to);
    ...
    try {
        const response = await axios_1.default.post(`https://api.mailgun.net/v3/${config_1.config.mailgunDomain}/messages`, form, {
            auth: {
                username: 'api',
                password: config_1.config.mailgunApiKey,
            },
        ...
        if (status === 401 || status === 403) {
            throw Object.assign(new Error(`Mailgun authentication error: ${detail}`), { statusCode: 502, retryable: false });
        }
        throw Object.assign(new Error(`Email dispatch failed (${status || 'network'}): ${detail}`), { statusCode: 502, retryable: status >= 500 || !status });
    }
}
/**
